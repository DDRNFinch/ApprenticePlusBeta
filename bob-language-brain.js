(() => {
  ...
  function outcomeMatches(assignmentNumber,message){
    ...
    if(nvq()){
      try{
        const criteria=(typeof NVQ_ASSESSMENT_CRITERIA!=='undefined'&&NVQ_ASSESSMENT_CRITERIA?.[String(a.unit)])||[];
        const byLo={};
        criteria.forEach(item=>{const s=scoreOutcome(item.text,message,hits);if(s>0)byLo[item.lo]=(byLo[item.lo]||0)+s});
        Object.entries(byLo).forEach(([code,score])=>{const row=rows.find(x=>x.code===code);if(row)row.score+=score;else rows.push({code,text:'',score})});
  ...
  function understand(message){
    const explicit=directMeaning(message);
    const fallback=!explicit?window.BobCourseBrain?.matchAssignment?.(message):null;
    const match=explicit||fallback||null;
    const outcomes=match?.n?outcomeMatches(match.n,message):[];
    return {version:VERSION,message,normalised:normalise(message),groups:groupHits(message),assignment:match?{n:match.n,title:match.title,unit:match.unit||''}:null,outcomes,confidence:explicit?'high':match?.confidence||'low'};
  }
  ...
  function install(){
    ...
    const originalMatch=window.BobCourseBrain.matchAssignment?.bind(window.BobCourseBrain);
    window.BobCourseBrain.matchAssignment=message=>directMeaning(message)||originalMatch?.(message)||null;
    window.BobCourseBrain.reply=message=>{
      const meaning=understand(message);
      const a=meaning.assignment;
      const q=normalise(message);
      const isGeneral=/^(hi|hello|hey|thanks|thank you|ok|okay|yes|no)\b/.test(q)||/progress|how far|what.*left|still need|evidence.*missing|what.*evidence/.test(q);
      if(a&&!isGeneral&&!/\b(unit|ep|evidence pack|assignment)\s*\d+\b/.test(q)){
        const hint=a.unit?` unit ${a.unit}`:` ep ${a.n}`;
        return originalReply(`${message}${hint}`);
      }
      return originalReply(message);
    };
    window.BobCourseBrain.language=understand;
    window.BobCourseBrain.__languageWrapped=true;
    return true;
  }

  function init(){if(!install())setTimeout(init,80)}
  window.BobLanguageBrain={version:VERSION,understand,outcomeMatches,groups:GROUPS};
  init();
})();