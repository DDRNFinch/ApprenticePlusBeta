(function(global){
  ...
  function showReceive(){
    const {modal,close}=showModal(`<div class="review-transfer-head"><div><div class="number">ASSESSOR+ REVIEW OUTCOME</div><h2>Receive completed review</h2><p>Choose the .apreview file sent by the assessor, or paste the transfer text.</p></div><button class="modal-close" data-review-transfer-close aria-label="Close">×</button></div><label class="field full"><span>Review file</span><input class="input" id="reviewOutcomeFile" type="file" accept=".apreview,.json,application/json"></label><label class="field full"><span>Or paste transfer text</span><textarea class="input" id="reviewOutcomeText" rows="6" placeholder="Paste the Assessor+ review package here"></textarea></label><div class="btn-row"><button class="btn secondary" data-review-transfer-close>Cancel</button><button class="btn" id="importReviewOutcome">Import review</button></div>`);
    modal.querySelector('#importReviewOutcome').onclick=async()=>{try{let text=modal.querySelector('#reviewOutcomeText').value.trim(),file=modal.querySelector('#reviewOutcomeFile').files?.[0];if(file)text=await file.text();if(!text)throw new Error('Choose a review file or paste the transfer text.');const outcome=await importOutcome(text);close();toast?.('Completed review imported');state.reviewMateTab='targets';state.reviewMateDraft=null;renderReviewMate();setTimeout(()=>showOutcome(outcome),50)}catch(error){toast?.(error.message||'The review could not be imported')}};
  }
