...
async function generateEvidencePackPDF({course, assignment, profile, sections, branding, returnPackage=false, newEvidence={}, assignmentRpl=false, rplKsbCodes=[], learningHours=null}) {
  ...
  function drawCoverImage(x,img,px,py,bw,bh){
    x.fillStyle=WHITE;x.strokeStyle='#E1E9E0';x.lineWidth=1;roundedRect(x,px,py,bw,bh,18);x.fill();x.stroke();
    if(!img)return;const scale=Math.max(bw/img.width,bh/img.height),iw=img.width*scale,ih=img.height*scale;x.save();roundedRect(x,px,py,bw,bh,18);x.clip();x.drawImage(img,px+(bw-iw)/2,py+(bh-ih)/2,iw,ih);x.restore();
  }
  ...
  const observationMedia=[];for(let vi=0;vi<(sections.practical||[]).length;vi++){const version=sections.practical[vi];for(const [code,media] of Object.entries(version.observationRecordings||{})){if(media?.video?.data)observationMedia.push({code,rec:media.video,attempt:vi+1,kind:'video'});if(media?.audio?.data)observationMedia.push({code,rec:media.audio,attempt:vi+1,kind:'audio'})}}
  if(evidenceFiles.length||walkthroughVideos.length||audios.length||observationMedia.length){
    ...
    observationMedia.forEach(({code,rec,attempt,kind})=>{const ext=mediaExtension(rec.type,rec.name||'',kind,rec.data),label=kind==='audio'?'Assessor Discussion':'Assessor Observation Video',base=safeZipName(`AO${attempt} - ${code} - ${label}`),name=uniqueMediaName(base,ext,used);entries.push({name:`Assessor Observation Recordings/${name}`,data:dataUrlBytes(rec.data)})});
...
async function generateNVQEvidencePackPDF({course, assignment, profile, sections, branding, returnPackage=false, newEvidence={}, assignmentRpl=false, rplKsbCodes=[], learningHours=null}) {
  ...
  const observationMedia=[];for(let vi=0;vi<(sections.practical||[]).length;vi++){const version=sections.practical[vi];for(const [code,media] of Object.entries(version.observationRecordings||{})){if(media?.video?.data)observationMedia.push({code,rec:media.video,attempt:vi+1,type:'video'});if(media?.audio?.data)observationMedia.push({code,rec:media.audio,attempt:vi+1,type:'audio'})}}
  if(walkthroughVideos.length||evidenceFiles.length||observationMedia.length){
    const entries=[{name:pdfName,data:pdf}],used=new Set();
    ...
    observationMedia.forEach(({code,rec,attempt,type},i)=>{const mime=String(rec.type||''),ext=mediaExtension(mime,rec.name||'',type,rec.data),label=type==='audio'?'Assessor Discussion':'Assessor Observation Video',base=safeZipName(`AO${attempt} - ${code} - ${label}`);let name=`${base}${ext}`;if(used.has(name.toLowerCase()))name=`${base}-${i+1}${ext}`;used.add(name.toLowerCase());entries.push({name:`Assessor Observation Recordings/${name}`,data:dataUrlBytes(rec.data)})});
