// V2.83 Apprentice+ current compiled evidence-pack export
'use strict';
...
const EVIDENCE_TYPE_STYLES=Object.freeze({
  photo:Object.freeze({label:'Learner Reflective Photographs',short:'LRP',colour:'#2563A8'}),
  statement:Object.freeze({label:'Learner Reflective Statement',short:'LRS',colour:'#7046A3'}),
  video:Object.freeze({label:'Learner Reflective Video',short:'LRV',colour:'#C94F45'}),
  voice:Object.freeze({label:'Learner Reflective Audio',short:'LRA',colour:'#087F78'}),
  documents:Object.freeze({label:'Supporting Document',short:'DOC',colour:'#A95D00'}),
  witness:Object.freeze({label:'Witness Testimony Statement',short:'WTS',colour:'#4F46A5'}),
  observation:Object.freeze({label:'Workplace Observation Statement',short:'WOS',colour:'#24613B'}),
  discussion:Object.freeze({label:'Workplace Observation Audio',short:'WOA',colour:'#056B8A'}),
  simulation:Object.freeze({label:'College Observation Statement',short:'COS',colour:'#8A4B22'}),
...
// Shared export palette. Renderers must consume these tokens rather than
// declaring renderer-local colours, so KSB and NVQ PDF paths cannot drift.
const PDF_STYLE_COLOURS=Object.freeze({
  apprenticeGreen:EVIDENCE_TYPE_STYLES.portfolio.colour,
  lightGreen:'#EAF7E4',
  softGreen:'#D9EFD0',
  ink:'#18231E',
  muted:'#68756D',
...
const EVIDENCE_TAXONOMY=Object.freeze({LRP:'Learner Reflective Photographs',LRS:'Learner Reflective Statement',LRV:'Learner Reflective Video',LRA:'Learner Reflective Audio',DOC:'Supporting Document',WTS:'Witness Testimony Statement',WOS:'Workplace Observation Statement',WOV:'Workplace Observation Video',WOA:'Workplace Observation Audio',COS:'College Observation Statement',COV:'College Observation Video',COA:'College Observation Audio',RPL:'Recognition of Prior Learning',OTJ:'Off-the-Job Training',GLH:'Guided Learning Hours'});
function evidenceReference(code,index){if(!EVIDENCE_TAXONOMY[code])throw new Error(`Unsupported evidence code: ${code}`);return `${code}-${String(Number(index)||1).padStart(2,'0')}`}
function mediaExportName(code,index,codes,extension){const ref=evidenceReference(code,index),label=EVIDENCE_TAXONOMY[code],mapped=[...new Set(codes||[])].filter(Boolean).join(' ')||'Unmapped',ext=String(extension||'webm').replace(/^\./,'').replace(/[^a-z0-9]/gi,'')||'webm';return `${ref} - ${label} - ${mapped}`.replace(/[\\/:*?"<>|]/g,'-').replace(/\s+/g,' ').trim()+`.${ext}`}
// observerRole is the value saved by the observation template.  Only records
// which pre-date that field use the former explicit college-context fallback.
function observationContext(record={}){const role=String(record.observerRole||'').trim().toLowerCase();if(role==='tutor')return 'college';if(role==='assessor')return 'workplace';return /college|simulation/i.test(`${record.location||''} ${record.context||''} ${record.observationType||''}`)?'college':'workplace'}
function observationEvidenceCodes(record={}){const prefix=observationContext(record)==='college'?'CO':'WO';return Object.freeze({statement:`${prefix}S`,video:`${prefix}V`,audio:`${prefix}A`})}
function nvqEvidenceCodes(recordType,record={}){const type=String(recordType||'').toLowerCase();if(type==='observation')return observationEvidenceCodes(record);return Object.freeze({photos:'LRP',statement:'LRS',video:'LRV',audio:'LRA',document:'DOC',witness:'WTS',rpl:'RPL',glh:'GLH'}[type]||'')}
function compareCriterionCodes(a,b){const parse=value=>{const text=String(value?.code??value??'').trim().toUpperCase(),match=text.match(/^([KSB])\s*(\d+)(.*)$/);return match?{group:{K:0,S:1,B:2}[match[1]],number:Number(match[2]),tail:match[3]}:{group:3,number:Number(text.match(/\d+/)?.[0]??Number.MAX_SAFE_INTEGER),tail:text}};const left=parse(a),right=parse(b);return left.group-right.group||left.number-right.number||left.tail.localeCompare(right.tail,undefined,{numeric:true})}
function compareAssessmentCriterionCodes(a,b){return String(a?.code??a??'').localeCompare(String(b?.code??b??''),undefined,{numeric:true,sensitivity:'base'})}
function assessmentCriteriaForOutcome(assignment,lo){
  const source=assignment?.criteria?.[lo];
  if(Array.isArray(source))return source.map(item=>typeof item==='string'?{code:item,text:''}:{code:String(item?.code||item?.number||''),text:String(item?.text||item?.wording||item?.description||'')}).filter(item=>item.code).sort(compareAssessmentCriterionCodes);
  if(source&&typeof source==='object')return Object.entries(source).map(([code,text])=>({code,text:String(text||'')})).sort(compareAssessmentCriterionCodes);
  return String(source||'').split(/\s*[•,;]\s*/).map(code=>({code:code.trim(),text:''})).filter(item=>item.code).sort(compareAssessmentCriterionCodes);
}
function buildNvqMatrixGroups(assignment,evidenceByOutcome={},required=2){return (assignment?.ksbs||[]).map(([code,text])=>{const mapped=evidenceByOutcome[code]||[],refs=[...new Set(mapped.map(item=>typeof item==='string'?item:item?.ref).filter(Boolean))],types=[...new Set(mapped.map(item=>typeof item==='string'?String(item).split('-')[0]:item?.type).filter(Boolean))],met=mapped.complete===true||mapped.rpl===true||refs.some(ref=>/^RPL(?:-|$)/.test(ref))||refs.length>=required;return {code,text,met,references:refs,evidenceTypes:types,criteria:assessmentCriteriaForOutcome(assignment,code).map(ac=>({...ac,met,references:refs,evidenceTypes:types}))}})}
function evidencePackTitleModel({course,assignment,profile,complete=false,completionDate=''}){return {code:String(assignment?.unit||`EP${assignment?.n||''}`),packCode:`EP${assignment?.n||''}`,title:String(assignment?.title||'Evidence Pack'),learner:String(profile?.fullName||profile?.name||'Learner'),courseName:String(course?.name||''),complete:!!complete,status:complete?'Complete':'In Progress',completionDate:complete&&completionDate?String(completionDate):''}}
function estimateNvqMatrixLayout(groups,{lineChars=82,linesPerPage=43}={}){let pages=1,used=0;const blocks=[];for(const group of groups||[]){const loLines=Math.max(1,Math.ceil(String(group.text||'').length/lineChars)),firstLines=group.criteria?.length?Math.max(3,Math.ceil(String(group.criteria[0].text||'').length/lineChars)+2):0;if(used&&used+loLines+firstLines>linesPerPage){pages++;used=0}used+=loLines+1;for(const ac of group.criteria||[]){const lines=Math.max(3,Math.ceil(String(ac.text||'').length/lineChars)+2+Math.ceil((ac.references||[]).join(', ').length/lineChars));if(used&&used+lines>linesPerPage){pages++;used=0}blocks.push({page:pages,lo:group.code,code:ac.code,lines});used+=lines} }return {pageCount:pages,blocks}}
function witnessRatingRows(record,criteria=[]){const lookup=new Map(criteria),scores=record?.scores||record?.ratings||{};return Object.entries(scores).filter(([,rating])=>Number(rating)>=1&&Number(rating)<=3).map(([key,rating])=>({label:record?.ratingLabels?.[key]||lookup.get(String(key).split('::')[0])||key,rating:Number(rating)}))}
function collectPortfolioMedia(sections={}){const output=[];const add=(code,event,codes,record)=>{if(record?.data)output.push({code,reference:evidenceReference(code,event),codes:[...new Set(codes||[])],record,name:mediaExportName(code,event,codes,String(record.type||record.name||'').includes('mp4')?'mp4':'webm')})};
  (sections.walkthrough||[]).forEach((r,i)=>add('LRV',i+1,r.confirmedCodes||r.intendedCodes||[r.code],r));
  (sections.discussion||[]).forEach((d,i)=>Object.values(d.recordings||{}).forEach(r=>add('LRV',i+1,d.ksbEvidence||Object.keys(d.recordings||{}),r)));
  (sections.professionalDiscussion||[]).forEach((d,i)=>{Object.values(d.recordings||{}).forEach(r=>add('LRA',i+1,d.ksbEvidence||Object.keys(d.recordings||{}),r));(d.voiceSubmissions||[]).forEach(r=>add('LRA',i+1,r.confirmedCodes||r.intendedCodes,r))});
  (sections.practical||[]).forEach((d,i)=>{const codes=observationEvidenceCodes(d);Object.entries(d.observationRecordings||{}).forEach(([code,pair])=>{add(codes.video,i+1,[code],pair?.video);add(codes.audio,i+1,[code],pair?.audio)})});return output}
function evidenceTypeStyle(value){
  const t=String(value||'').toLowerCase();
  if(/recognition of prior learning|\brpl\b/.test(t))return EVIDENCE_TYPE_STYLES.rpl;
  if(/college observation|college simulation|\bco[asv]\b|\bcs\b/.test(t))return EVIDENCE_TYPE_STYLES.simulation;
  if(/workplace observation|\bwo[asv]\b/.test(t))return EVIDENCE_TYPE_STYLES.observation;
  ...
  if(/talk about it|professional discussion|\bpda?\b|\bpds\b/.test(t))return EVIDENCE_TYPE_STYLES.voice;
...
function paintEvidencePackDividerBackground(ctx,W,H){ctx.fillStyle=PDF_STYLE_COLOURS.apprenticeGreen;ctx.fillRect(0,0,W,H)}

/* Apprentice+ offline PDF generator. Evidence stays in the browser; no data is uploaded. */
async function generateEvidencePackPDF({course, assignment, profile, sections, branding, returnPackage=false, newEvidence={}, assignmentRpl=false, rplKsbCodes=[], rplEvidence=null, learningHours=null, completionStatus=false, completionDate=''}) {
  if(course && course.nvqUnits) return generateNVQEvidencePackPDF({course, assignment, profile, sections, branding, returnPackage, newEvidence, assignmentRpl, rplKsbCodes, rplEvidence, learningHours, completionStatus, completionDate});
  const W=1240,H=1754,M=88,TEAL=PDF_STYLE_COLOURS.apprenticeGreen,GREEN=TEAL,LIGHT_GREEN=PDF_STYLE_COLOURS.lightGreen,SOFT_GREEN=PDF_STYLE_COLOURS.softGreen,INK=PDF_STYLE_COLOURS.ink,MUTED=PDF_STYLE_COLOURS.muted,PALE=PDF_STYLE_COLOURS.pale,WHITE=PDF_STYLE_COLOURS.white;
  const pages=[];
  ...
  (sections.practical||[]).forEach((d,i)=>{const ref=evidenceReference(observationEvidenceCodes(d).statement,i+1);addEvidence(ref,`Verified by Someone Else ${i+1}`,'Assessor Observation',d.date);practicalCodes(d).forEach(code=>addMatrix(code,ref))});
  (sections.statement||[]).forEach((d,i)=>{const ref=evidenceReference('LRS',i+1);addEvidence(ref,`Write About It ${i+1}`,'Learner Statement',d.date);selectedCodes(d).forEach(code=>addMatrix(code,ref))});
  (sections.witness||[]).forEach((d,i)=>{const ref=evidenceReference('WTS',i+1);addEvidence(ref,`Verified by Someone Else ${i+1}`,'Witness Testimony',d.date);selectedCodes(d).forEach(code=>addMatrix(code,ref))});
  (sections.photos||[]).forEach((d,i)=>{const ref=evidenceReference('LRP',i+1);addEvidence(ref,`Take Photos ${i+1}`,'Photo Evidence',d.date);selectedCodes(d).forEach(code=>addMatrix(code,ref))});
  (sections.discussion||[]).forEach((d,i)=>{const ref=evidenceReference('LRV',i+1);addEvidence(ref,`Record a Video ${i+1}`,'Video Walkthrough',d.date);selectedCodes(d).forEach(code=>addMatrix(code,ref))});
  (sections.professionalDiscussion||[]).forEach((d,i)=>{const ref=evidenceReference('LRA',i+1);addEvidence(ref,`Talk About It ${i+1}`,'Professional Discussion',d.date);Object.keys(d.recordings||{}).filter(code=>d.recordings?.[code]?.data).forEach(code=>addMatrix(code,ref));(d.voiceSubmissions||[]).forEach(rec=>(rec.confirmedCodes||rec.intendedCodes||[]).forEach(code=>addMatrix(code,ref))) });
  (sections.walkthrough||[]).forEach((d,i)=>{const ref=evidenceReference('LRV',i+1);addEvidence(ref,`Record a Video ${i+1}`,'Video Walkthrough',d.date);if(d.code)addMatrix(d.code,ref)});
  (sections.supporting||[]).forEach((d,i)=>{const ref=evidenceReference('DOC',i+1);addEvidence(ref,`Upload Evidence ${i+1}`,d.type||'Supporting Evidence',d.date);selectedCodes(d).forEach(code=>addMatrix(code,ref))});
  ...
  {
    const c=document.createElement('canvas');c.width=W;c.height=H;const x=c.getContext('2d');paintEvidencePackDividerBackground(x,W,H);
    const divider=evidencePackTitleModel({course,assignment,profile,complete:completionStatus,completionDate});
    x.fillStyle=WHITE;x.font='800 18px Arial';x.fillText('APPRENTICE+ · EVIDENCE PACK',M,300);
    x.font='900 52px Arial';x.fillText(divider.packCode,M,382);
    x.font='700 58px Arial';fitText(x,clean(divider.title),M,485,W-2*M,58);
    let y=640;const rows=[['Learner',divider.learner],['Course / qualification',divider.courseName],['Status',divider.completionDate?`Completed · ${divider.completionDate}`:divider.status]];
    rows.forEach(([a,b])=>{x.fillStyle=WHITE;x.font='600 13px Arial';x.fillText(clean(a).toUpperCase(),M,y);x.font='600 21px Arial';fitText(x,clean(b||'-'),M,y+31,W-2*M,21);y+=92});
  ...
  // Page 1 - portfolio contents
  {
    ...
    const keys=['WOS/COS  Observation Statement','LRS  Learner Reflective Statement','WTS  Witness Testimony Statement','LRV  Learner Reflective Video','LRA  Learner Reflective Audio','LRP  Learner Reflective Photographs','DOC  Supporting Document'];
  ...
  for(let i=0;i<(sections.photos||[]).length;i++){
    ...
    await addFixedPhotoEvidenceRecord(d,{title:`Take Photos · LRP-${String(v).padStart(2,'0')}`,version:v,date:d.date,type:'Learner Reflective Photographs',signatureTitle:'Learner signature',assessor:false,summaryFields:[['Activity / photo notes',d.activity],['KSBs met by this evidence',selectedKsbDetails(d)]]});
  }

  // Statements
  for(let i=0;i<(sections.statement||[]).length;i++){
    const d=sections.statement[i],v=i+1;await addCompleteEvidenceRecord({title:`Write About It · LRS-${String(v).padStart(2,'0')}`,version:v,date:d.date,type:'Learner Reflective Statement',fields:[['KSBs met by this evidence',selectedKsbDetails(d)],['Learner response',d.text||d.statement||d.learnerStatement]],signatureTitle:'Learner signature',signatureData:d.signature});
  }

  for(let i=0;i<(sections.discussion||[]).length;i++){
    const d=sections.discussion[i],v=i+1;await addCompleteEvidenceRecord({title:`Record a Video · LRV-${String(v).padStart(2,'0')}`,version:`Attempt ${v}`,date:d.date,type:'Learner Reflective Video',newEvidenceType:'discussion',fields:[['Walkthrough lead',d.assessor],['Activity demonstrated',d.activity],['Video evidence and notes',recordingDetails(d,'Video recording')]],signatureTitle:'Assessor / walkthrough lead signature',signatureData:d.signature});
  }
  ...
  for(let i=0;i<(sections.witness||[]).length;i++){
    ...
    await addCompleteEvidenceRecord({title:`Witness Testimony · WTS-${String(v).padStart(2,'0')}`,version:`Attempt ${v}`,date:d.date,type:'Witness Testimony Statement',fields:[['Evidence source',d.type],['Name',d.personName],['Role',d.role],['Preferred contact details',d.contactDetails||d.organisation],['Activity witnessed',d.activity],['KSBs met by this evidence',selectedKsbDetails(d)],['Ratings',witnessRatingDetails(d)],['Witness Testimony / Additional Comments',d.feedback],['Additional Comments',d.additionalComments]],signatureTitle:`${d.type||'Witness'} signature`,signatureData:d.signature});
    await addFilePreviewPages(d,`Witness Testimony · WTS-${String(v).padStart(2,'0')}`,`Attempt ${v}`,d.date,'Witness Testimony');
  }
  ...
  for(let i=0;i<(sections.walkthrough||[]).length;i++){
    ...
    await addCompleteEvidenceRecord({title:`Record a Video · LRV-${String(v).padStart(2,'0')}`,version:v,date:f.date,type:'Learner Reflective Video',newEvidenceType:'walkthrough',fields:[['KSB met by this evidence',[f.code,f.summary].filter(Boolean).join(' - ')],['Video filename',ksbMediaFileName(f)],['Original filename',f.name],['Recorded date',f.date],['Duration',f.duration],['Media format',f.type],['File size',Number(f.size)>0?`${Math.round(Number(f.size)/1024)} KB`:'']]});
  }
  ...
  footerAll();
  if(assignmentRpl)pages.filter(p=>!p.isCover).forEach(p=>drawRecordedPriorLearningStamp(p.ctx));
  ...
  const portfolioMedia=collectPortfolioMedia(sections);
  if(evidenceFiles.length||portfolioMedia.length){
    ...
    portfolioMedia.forEach(media=>entries.push({name:`Media/${media.name}`,data:dataUrlBytes(media.record.data)}));
...
async function generateNVQEvidencePackPDF({course, assignment, profile, sections, branding, returnPackage=false, newEvidence={}, assignmentRpl=false, rplKsbCodes=[], rplEvidence=null, learningHours=null, completionStatus=false, completionDate=''}) {
  const W=1240,H=1754,M=88,TEAL=PDF_STYLE_COLOURS.apprenticeGreen,GREEN=TEAL,SOFT_GREEN=PDF_STYLE_COLOURS.softGreen,YELLOW='#F7D75C',GREY='#EEF5EE',INK=PDF_STYLE_COLOURS.ink,MUTED=PDF_STYLE_COLOURS.muted,PALE=PDF_STYLE_COLOURS.pale,WHITE=PDF_STYLE_COLOURS.white;
  const pages=[];
  const newEvidenceAliases={practical:'practical',practicalassessment:'practical',assessorobservation:'practical',photos:'photos',photoevidence:'photos',photographicevidence:'photos',statement:'statement',learnerstatement:'statement',discussion:'discussion',videowalkthrough:'walkthrough',professionaldiscussion:'professionalDiscussion',witness:'witness',witnesstestimony:'witness',supporting:'supporting',supportingevidence:'supporting',walkthrough:'walkthrough'};
  ...
  (sections.practical||[]).forEach((d,i)=>selectedCodes(d).forEach(c=>add(c,evidenceReference(observationEvidenceCodes(d).statement,i+1),EVIDENCE_TAXONOMY[observationEvidenceCodes(d).statement])));
  (sections.photos||[]).forEach((d,i)=>selectedCodes(d).forEach(c=>add(c,evidenceReference('LRP',i+1),EVIDENCE_TAXONOMY.LRP)));
  (sections.statement||[]).forEach((d,i)=>selectedCodes(d).forEach(c=>add(c,evidenceReference('LRS',i+1),EVIDENCE_TAXONOMY.LRS)));
  (sections.discussion||[]).forEach((d,i)=>selectedVideoCodes(d).forEach(c=>add(c,evidenceReference('LRV',i+1),EVIDENCE_TAXONOMY.LRV)));
  (sections.walkthrough||[]).forEach((d,i)=>{if(d.code)add(d.code,evidenceReference('LRV',i+1),EVIDENCE_TAXONOMY.LRV)});
  (sections.professionalDiscussion||[]).forEach((d,i)=>{selectedDiscussion(d).forEach(([c])=>add(c,evidenceReference('LRA',i+1),EVIDENCE_TAXONOMY.LRA));(d.voiceSubmissions||[]).forEach(rec=>(rec.confirmedCodes||rec.intendedCodes||[]).forEach(c=>add(c,evidenceReference('LRA',i+1),EVIDENCE_TAXONOMY.LRA))) });
  (sections.witness||[]).forEach((d,i)=>selectedCodes(d).forEach(c=>add(c,evidenceReference('WTS',i+1),EVIDENCE_TAXONOMY.WTS)));
  (sections.supporting||[]).forEach((d,i)=>selectedCodes(d).forEach(c=>add(c,evidenceReference('DOC',i+1),EVIDENCE_TAXONOMY.DOC)));
  rplCodes.forEach(code=>add(code,evidenceReference('RPL',1),EVIDENCE_TAXONOMY.RPL));
  ...
  {const divider=evidencePackTitleModel({course,assignment,profile,complete:completionStatus,completionDate}),p=newPage(`Unit ${divider.code}`,'APPRENTICE+ · NVQ EVIDENCE PACK'),x=p.x;paintEvidencePackDividerBackground(x,W,H);pages[pages.length-1].isDivider=true;x.fillStyle=WHITE;x.font='800 18px Arial';x.fillText('UNIT',M,330);x.font='900 72px Arial';x.fillText(divider.code,M,420);x.font='700 54px Arial';const titleLines=wrap(x,divider.title,W-2*M,'700 54px Arial');titleLines.forEach((line,i)=>x.fillText(line,M,535+i*68));const y=535+titleLines.length*68+65;[['Learner',divider.learner],['Course / qualification',divider.courseName],['Status',divider.completionDate?`Completed · ${divider.completionDate}`:divider.status]].forEach(([a,b],i)=>{x.fillStyle=WHITE;x.font='700 12px Arial';x.fillText(clean(a).toUpperCase(),M,y+i*92);x.font='600 21px Arial';fitText(x,clean(b||'-'),M,y+34+i*92,W-2*M,21)})}

  // Learning Outcome Evidence Matrix: hierarchical, flow-layout matrix. Heights come from wrapped lines rather than fixed table cells.
  {const groups=buildNvqMatrixGroups(assignment,evidenceMap,Number(course.evidenceRequirement||2));let p,x,y;const start=continued=>{p=meta(newPage(`Unit ${assignment.unit} Evidence Matrix${continued?' - Continued':''}`,`${profile.fullName} · ${completionStatus?'Complete':'In Progress'}`),'-','Learning Outcome Matrix',1);x=p.x;y=sectionHeading(x,continued?'LO → AC → Supporting Evidence · continued':'LO → AC → Supporting Evidence',p.y)};start(false);
   for(const group of groups){x.font='700 20px Arial';const loLines=wrap(x,`${group.code} · ${group.text}`,W-2*M-32,'700 20px Arial'),first=group.criteria[0],firstNeed=first?Math.max(82,wrap(x,first.text||'',W-2*M-60,'400 16px Arial').length*22+72):0,loH=loLines.length*25+40;if(y+loH+firstNeed>H-112)start(true);x.fillStyle=SOFT_GREEN;x.fillRect(M,y,W-2*M,loH);x.fillStyle=INK;x.font='700 20px Arial';loLines.forEach((line,i)=>x.fillText(line,M+16,y+27+i*25));y+=loH+10;
    for(const ac of group.criteria){const wording=wrap(x,ac.text||'',W-2*M-60,'400 16px Arial'),refs=ac.references.length?ac.references.join(', '):'No mapped evidence',refLines=wrap(x,`Evidence: ${refs}`,W-2*M-60,'600 15px Arial'),h=Math.max(82,54+wording.length*22+refLines.length*21);if(y+h>H-112)start(true);x.fillStyle=ac.met?PALE:WHITE;x.fillRect(M,y,W-2*M,h-6);x.strokeStyle='#DCE7DA';x.strokeRect(M+.5,y+.5,W-2*M-1,h-7);x.fillStyle=TEAL;x.font='800 18px Arial';x.fillText(ac.code,M+16,y+27);x.fillStyle=ac.met?GREEN:MUTED;x.font='800 16px Arial';x.fillText(ac.met?'✓ Met':'— Not yet met',M+130,y+27);x.fillStyle=INK;x.font='400 16px Arial';wording.forEach((line,i)=>x.fillText(line,M+16,y+54+i*22));x.fillStyle=MUTED;x.font='600 15px Arial';refLines.forEach((line,i)=>x.fillText(line,M+16,y+54+wording.length*22+i*21));y+=h}
   }
   if(y+130>H-110){p=meta(newPage('Learning Outcome Evidence Key','Automatic mapping'),'-','Learning Outcome Matrix',1);x=p.x;y=sectionHeading(x,'Evidence Reference Key',p.y)}else y=sectionHeading(x,'Evidence Reference Key',y+12);const keys=['WOS/COS  Observation Statement','LRS  Learner Reflective Statement','WTS  Witness Testimony Statement','LRV  Learner Reflective Video','LRA  Learner Reflective Audio','LRP  Learner Reflective Photographs','DOC  Supporting Document','RPL  Recognition of Prior Learning','GLH  Guided Learning Hours'];keys.forEach((t,i)=>{x.fillStyle=INK;x.font='600 16px Arial';x.fillText(t,M+(i%2)*520,y+Math.floor(i/2)*32)})}

  // Assessor observations preserve the complete narrative; optional photographs follow on a dedicated 3x3 page.
  for(let i=0;i<(sections.practical||[]).length;i++){const d=sections.practical[i],attempt=i+1;await addCompleteNvqRecord({title:`${EVIDENCE_TAXONOMY[observationEvidenceCodes(d).statement]} · ${evidenceReference(observationEvidenceCodes(d).statement,attempt)}`,attempt,date:d.date,type:EVIDENCE_TAXONOMY[observationEvidenceCodes(d).statement],fields:[[d.observerRole||'Observer',d.tutor||d.assessor],['Activity observed',d.activity],['Learning Outcomes met by this evidence',selectedKsbDetails(d)],['Full assessor observation',d.feedback||d.observation||d.observationText],['Assessment summary',d.feedbackSummary],['Areas for improvement',d.feedbackDevelopment],['Additional assessor comments',d.additionalComments],['Assessor discussion / recordings',observationRecordingOutcomeDetails(d)]],signatureTitle:'Assessor signature',signatureData:d.signature});const hasPhotos=Object.values(d?.outcomePhotos||{}).some(photo=>photo?.data)||(d?.photos||[]).some(photo=>photo?.data);if(hasPhotos){await addCompactNvqAssessorRecord({...d,feedback:'',signature:''},attempt)}}
  for(let i=0;i<(sections.photos||[]).length;i++){const d=sections.photos[i],attempt=i+1;await addCompactNvqPhotoRecord(d,attempt)}
  for(let i=0;i<(sections.statement||[]).length;i++){const d=sections.statement[i],attempt=i+1;await addCompleteNvqRecord({title:`Write About It · ${evidenceReference('LRS',attempt)}`,attempt,date:d.date,type:'Learner Reflective Statement',fields:[['Learning Outcomes met by this evidence',selectedOutcomeDetails(d)],['Learner response',d.text||d.statement||d.learnerStatement]],signatureTitle:'Learner signature',signatureData:d.signature})}
  for(let i=0;i<(sections.discussion||[]).length;i++){const d=sections.discussion[i],attempt=i+1,codes=selectedVideoCodes(d);await addCompleteNvqRecord({title:`Record a Video · ${evidenceReference('LRV',attempt)}`,attempt,date:d.date,type:'Learner Reflective Video',newEvidenceType:'discussion',fields:[['Learning Outcomes met by this evidence',codes.map(code=>{const row=(assignment.ksbs||[]).find(([itemCode])=>itemCode===code);return `${code} - ${row?.[1]||'Learning Outcome video'}`}).join('\n')],['Video recordings',videoRecordingOutcomeDetails(d)] ]})}
  for(let i=0;i<(sections.professionalDiscussion||[]).length;i++){const d=sections.professionalDiscussion[i],attempt=i+1;await addCompleteNvqRecord({title:`Learner Reflective Audio · ${evidenceReference('LRA',attempt)}`,attempt,date:d.date,type:'Learner Reflective Audio',fields:[['Discussion lead',d.assessor],['Activity discussed',d.activity],['Learning Outcome recordings and notes',recordingOutcomeDetails(d,'Audio recording')]],signatureTitle:'Assessor / discussion lead signature',signatureData:d.signature})}
  for(let i=0;i<(sections.witness||[]).length;i++){const d=sections.witness[i],attempt=i+1,outcomes=selectedScores(d);await addCompleteNvqRecord({title:`Witness Testimony · ${evidenceReference('WTS',attempt)}`,attempt,date:d.date,type:'Witness Testimony Statement',fields:[['Evidence source',d.type],['Name',d.personName],['Role',d.role],['Preferred contact details',d.contactDetails||d.organisation],['Activity witnessed',d.activity],['Learning Outcomes witnessed',selectedOutcomeDetails(d)],['All marks awarded',scoredOutcomeDetails(d)],['Witness Testimony / Additional Comments',d.feedback],['Additional Comments',d.additionalComments]],signatureTitle:'Witness signature',signatureData:d.signature})}
  for(let i=0;i<(sections.supporting||[]).length;i++){const d=sections.supporting[i],attempt=i+1;await addCompleteNvqRecord({title:`Supporting Document · ${evidenceReference('DOC',attempt)}`,attempt,date:d.date,type:'Supporting Evidence',fields:[['Evidence tab',d.tab],['Evidence type',d.type],['Name',d.personName],['Role',d.role],['Organisation',d.organisation],['Activity / description',d.activity],['Learning Outcomes met by this evidence',selectedOutcomeDetails(d)],['All marks awarded',scoredOutcomeDetails(d)],['Assessment Summary',d.feedbackSummary],['Areas for Improvement',d.feedbackDevelopment],['Additional Comments',d.feedback],['Attached files',fileOutcomeDetails(d)],['Photographs and captions',photoOutcomeDetails(d)]],signatureTitle:`${d.type||'Supporting evidence'} signature`,signatureData:d.signature})}

  for(let i=0;i<(sections.walkthrough||[]).length;i++){const rec=sections.walkthrough[i],attempt=i+1;await addCompleteNvqRecord({title:`Record a Video · ${evidenceReference('LRV',attempt)}`,attempt,date:rec.date,type:'Learner Reflective Video',newEvidenceType:'walkthrough',fields:[['Learning Outcome met by this evidence',[rec.code,rec.summary].filter(Boolean).join(' - ')],['Video filename',ksbMediaFileName(rec)],['Original filename',rec.name],['Recorded date',rec.date],['Duration',rec.duration],['Media format',rec.type],['File size',Number(rec.size)>0?`${Math.round(Number(rec.size)/1024)} KB`:'']]})}
  ...
  const numbered=pages.filter(p=>!p.isDivider),total=numbered.length;let pageNo=0;pages.forEach(p=>{if(p.isDivider)return;pageNo++;const x=p.ctx;x.fillStyle='#DCE7DA';x.fillRect(M,H-72,W-2*M,1);x.fillStyle=MUTED;x.font='500 13px Arial';x.fillText('Apprentice+ · NVQ Evidence Pack',M,H-34);x.textAlign='right';x.fillText(`${pageNo} / ${total}`,W-M,H-34);x.textAlign='left'});
  if(assignmentRpl)pages.filter(p=>!p.isDivider).forEach(p=>drawRecordedPriorLearningStamp(p.ctx));
...
if(typeof module!=='undefined'&&module.exports)module.exports={EVIDENCE_TYPE_STYLES,PDF_STYLE_COLOURS,EVIDENCE_TAXONOMY,evidenceTypeStyle,evidenceReference,observationContext,observationEvidenceCodes,nvqEvidenceCodes,compareCriterionCodes,compareAssessmentCriterionCodes,assessmentCriteriaForOutcome,buildNvqMatrixGroups,evidencePackTitleModel,estimateNvqMatrixLayout,mediaExportName,witnessRatingRows,collectPortfolioMedia,takePhotosGridLayout,compileUnitPack,compileFullPortfolioFromUnitPacks,buildKsbMatrixRows,paintEvidencePackDividerBackground,generateEvidencePackPDF,generateNVQEvidencePackPDF,makeImagePDF,makeZipBlob};
