...
async function generateEvidencePackPDF({course, assignment, profile, sections, branding, returnPackage=false, newEvidence={}, assignmentRpl=false, rplKsbCodes=[]}) {
  ...
  function observationRecordingDetails(d){return Object.entries(d?.observationRecordings||{}).map(([code,media])=>{const parts=[];if(media?.audio?.data)parts.push(`Professional discussion recorded${media.audio.duration?` - ${media.audio.duration}`:''}`);if(media?.video?.data)parts.push(`Video recorded${media.video.duration?` - ${media.video.duration}`:''}`);return parts.length?`${code}: ${parts.join(' | ')}`:''}).filter(Boolean).join('\n')}
