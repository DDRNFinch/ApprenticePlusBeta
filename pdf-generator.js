...
async function generateEvidencePackPDF({course, assignment, profile, sections, branding, returnPackage=false, newEvidence={}, assignmentRpl=false, rplKsbCodes=[]}) {
  ...
  const PDF_COLOURS={cover:TEAL,practical:'#79d22f',statement:'#79d22f',witness:'#79d22f',video:'#79d22f',discussion:'#79d22f',photo:'#79d22f',knowledge:'#79d22f',documents:'#79d22f',specification:'#79d22f'};
  ...
  const pdfStampFor=title=>{const t=String(title||'').toLowerCase();if(/ksb evidence matrix|ksb mapping/.test(t))return ['KSB','Evidence Matrix',TEAL];if(/portfolio contents|evidence index/.test(t))return ['CONT','Contents',TEAL];if(/practical assessment|assessor observation/.test(t))return ['VER','Verified Evidence',PDF_COLOURS.practical];if(/learner statement/.test(t))return ['WRITE','Write About It',PDF_COLOURS.statement];if(/witness|employer|supporting evidence - statement/.test(t))return ['VER','Verified Evidence',PDF_COLOURS.witness];if(/video|walkthrough/.test(t))return ['VIDEO','Record a Video',PDF_COLOURS.video];if(/professional discussion/.test(t))return ['TALK','Talk About It',PDF_COLOURS.discussion];if(/photo|image/.test(t))return ['PHOTO','Take Photos',PDF_COLOURS.photo];if(/knowledge|theory|question/.test(t))return ['KQ','Knowledge Questions',PDF_COLOURS.knowledge];if(/specification/.test(t))return ['SPEC','Specification',PDF_COLOURS.specification];if(/document|certificate|supporting evidence/.test(t))return ['FILE','Upload Evidence',PDF_COLOURS.documents];if(/assignment/.test(t))return ['EV','Evidence Pack',TEAL];return ['PORT','Portfolio',TEAL]};
  const drawPdfStamp=(x,title,cx=W-142,cy=108,r=70)=>{const [code,labelText,colour]=pdfStampFor(title);const text=clean(`${code}  ${labelText}`.toUpperCase()),pad=18;x.save();x.font='700 13px Arial';const pillW=Math.min(238,Math.max(104,x.measureText(text).width+pad*2)),pillH=32,px=W-M-pillW,py=46;x.fillStyle='#ffffff';x.strokeStyle='#D9DEDC';x.lineWidth=1;x.fillRect(px,py,pillW,pillH);x.strokeRect(px+.5,py+.5,pillW-1,pillH-1);x.fillStyle=colour;x.fillRect(px,py,4,pillH);x.fillStyle='#3A4445';x.textAlign='center';x.textBaseline='middle';x.fillText(text,px+pillW/2+2,py+pillH/2);x.textAlign='left';x.textBaseline='alphabetic';x.restore()};
  ...
  const newPage=(title,version,status='Submitted - Locked')=>{
    ...
    x.fillStyle='#D9DEDC';x.fillRect(M,178,W-2*M,1);
    ...
    x.font='600 13px Arial';const tabW=Math.min(230,Math.max(125,x.measureText(sectionName.toUpperCase()).width+34)),tabX=W-M-tabW; x.fillStyle='#F6F7F6';x.fillRect(tabX,191,tabW,30);x.strokeStyle='#E2E6E4';x.lineWidth=1;x.strokeRect(tabX+.5,191.5,tabW-1,29);x.fillStyle=sectionColour;x.fillRect(tabX,191,3,30);x.fillStyle='#4E5859';x.textAlign='center';x.fillText(sectionName.toUpperCase(),tabX+tabW/2+2,211);x.textAlign='left';
  ...
  function meta(p,version,date,type,newEvidenceType=type){const {x}=p;let y=p.y;x.fillStyle='#F7F9F8';x.fillRect(M,y,W-2*M,82);x.strokeStyle='#E3E6E5';x.lineWidth=1;x.strokeRect(M+.5,y+.5,W-2*M-1,81);label(x,'Learner',M+18,y+23);value(x,profile.fullName,M+18,y+48,17,true);label(x,'Date',M+520,y+23);value(x,date||'-',M+520,y+48,17,true);label(x,'Evidence',M+750,y+23);value(x,p.x._sectionName||type,M+750,y+48,16,true);if(isPortfolioBuildingEvidence(type)){x.fillStyle='#5F6F70';x.font='700 11px Arial';x.fillText('PORTFOLIO BUILDING · 0.2 OTJ HOURS',M+18,y+70)}if(isNewEvidence(newEvidenceType,version))drawNewEvidenceStamp(x);p.y=y+104;return p}
...
async function generateNVQEvidencePackPDF({course, assignment, profile, sections, branding, returnPackage=false, newEvidence={}, assignmentRpl=false, rplKsbCodes=[]}) {
  ...
  const PDF_COLOURS={cover:'#79d22f',practical:'#79d22f',statement:'#79d22f',witness:'#79d22f',video:'#79d22f',discussion:'#79d22f',photo:'#79d22f',knowledge:'#79d22f',documents:'#79d22f'};
  ...
  const pdfStampFor=title=>{const t=String(title||'').toLowerCase();if(/learning outcome|coverage|evidence pack/.test(t))return ['NVQ','Evidence Pack',PDF_COLOURS.cover];if(/assessor observation|practical/.test(t))return ['VER','Verified Evidence',PDF_COLOURS.practical];if(/learner statement/.test(t))return ['WRITE','Write About It',PDF_COLOURS.statement];if(/witness|employer/.test(t))return ['VER','Verified Evidence',PDF_COLOURS.witness];if(/video|walkthrough/.test(t))return ['VIDEO','Record a Video',PDF_COLOURS.video];if(/professional discussion/.test(t))return ['TALK','Talk About It',PDF_COLOURS.discussion];if(/photo|image/.test(t))return ['PHOTO','Take Photos',PDF_COLOURS.photo];if(/knowledge|question/.test(t))return ['KQ','Knowledge Questions',PDF_COLOURS.knowledge];if(/document|certificate|supporting/.test(t))return ['FILE','Upload Evidence',PDF_COLOURS.documents];return ['PORT','Portfolio',PDF_COLOURS.cover]};
  ...
  const total=pages.length;pages.forEach((p,i)=>{const x=p.ctx;x.fillStyle='#D9DEDC';x.fillRect(M,H-72,W-2*M,1);x.fillStyle=MUTED;x.font='500 13px Arial';x.fillText('Apprentice+ · NVQ Evidence Pack',M,H-34);x.textAlign='right';x.fillText(`${i+1} / ${total}`,W-M,H-34);x.textAlign='left'});
  if(assignmentRpl)pages.forEach(p=>drawRecordedPriorLearningStamp(p.ctx));
