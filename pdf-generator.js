...
async function generateEvidencePackPDF({course, assignment, profile, sections, branding, returnPackage=false, newEvidence={}, assignmentRpl=false, rplKsbCodes=[]}) {
  ...
  const newPage=(title,version,status='Submitted - Locked')=>{
    ...
    drawLogo(x,apprenticeLogo,M,52,150,66);
    if(collegeLogo)drawLogo(x,collegeLogo,W-M-430,52,165,58);
    drawPdfStamp(x,title);
    x.fillStyle=MUTED;x.font='600 14px Arial';x.fillText(`${clean(course.standard||'')}  ·  Level ${clean(course.level||'-')}`,M,148);
    x.fillStyle=INK;x.font='700 35px Arial';fitText(x,clean(course.name||'Apprenticeship Course'),M,194,W-2*M,35);
    x.fillStyle='#D9DEDC';x.fillRect(M,222,W-2*M,1);
    x.fillStyle=INK;x.font='700 28px Arial';fitText(x,clean(title),M,274,W-2*M-250,28);
    const metaText=`${String(version).startsWith('Attempt')?version:`Version ${version}`}  ·  ${status}`;x.fillStyle=MUTED;x.font='500 15px Arial';x.fillText(metaText,M,306);
    x.font='600 13px Arial';const tabW=Math.min(230,Math.max(125,x.measureText(sectionName.toUpperCase()).width+34)),tabX=W-M-tabW; x.fillStyle='#F6F7F6';x.fillRect(tabX,247,tabW,34);x.strokeStyle='#E2E6E4';x.lineWidth=1;x.strokeRect(tabX+.5,247.5,tabW-1,33);x.fillStyle=sectionColour;x.fillRect(tabX,247,3,34);x.fillStyle='#4E5859';x.textAlign='center';x.fillText(sectionName.toUpperCase(),tabX+tabW/2+2,269);x.textAlign='left';
    pages.push({canvas:c,ctx:x,colour:sectionColour,sectionName});return {c,x,y:342};
  };
  ...
  function line(x,y,w= W-2*M){x.fillStyle='#D9DEDC';x.fillRect(M,y,w,1)}
  ...
  function meta(p,version,date,type,newEvidenceType=type){const {x}=p;let y=p.y;x.fillStyle='#FAFBFA';x.fillRect(M,y,W-2*M,118);x.strokeStyle='#E3E6E5';x.lineWidth=1;x.strokeRect(M+.5,y+.5,W-2*M-1,117);label(x,'Learner',M+22,y+28);value(x,profile.fullName,M+22,y+56,19,true);label(x,'Date submitted',M+570,y+28);value(x,date||'-',M+570,y+56,19,true);label(x,'Evidence type',M+22,y+86);value(x,type,M+180,y+86,17,true);label(x,'Status',M+570,y+86);value(x,'Submitted - Locked',M+650,y+86,17,true);if(isPortfolioBuildingEvidence(type)){x.fillStyle='#6D28D9';x.font='700 12px Arial';x.fillText('PORTFOLIO BUILDING · 0.2 OTJ HOURS',M+22,y+108)}if(isNewEvidence(newEvidenceType,version))drawNewEvidenceStamp(x);p.y=y+148;return p}
  ...
  function footerAll(){const numbered=pages.filter(p=>!p.isCover),total=numbered.length;let pageNo=0;pages.forEach(p=>{if(p.isCover)return;pageNo++;const x=p.ctx;x.fillStyle='#D9DEDC';x.fillRect(M,H-72,W-2*M,1);x.fillStyle=MUTED;x.font='500 13px Arial';x.fillText(`${branding?.name?branding.name+'  ·  ':''}Apprentice+`,M,H-34);x.textAlign='right';x.fillText(`${pageNo} / ${total}`,W-M,H-34);x.textAlign='left'})}
  ...
  (sections.practical||[]).forEach((d,i)=>{const ref=`AO${i+1}`;addEvidence(ref,`Assessor Observation ${i+1}`,'Assessor Observation',d.date);practicalCodes(d).forEach(code=>addMatrix(code,ref))});
  ...
  // Front cover (unnumbered)
  {
    ...
    x.fillStyle='#D9DEDC';x.fillRect(M,208,W-2*M,1);
...
async function generateNVQEvidencePackPDF({course, assignment, profile, sections, branding, returnPackage=false, newEvidence={}, assignmentRpl=false, rplKsbCodes=[]}) {
  ...
  const drawPdfStamp=(x,title,cx=W-138,cy=104,r=68)=>{const [code,labelText,colour]=pdfStampFor(title);const text=clean(`${code}  ${labelText}`.toUpperCase()),pad=18;x.save();x.font='700 13px Arial';const pillW=Math.min(250,Math.max(104,x.measureText(text).width+pad*2)),pillH=36,px=W-M-pillW,py=72;x.fillStyle='#ffffff';x.strokeStyle='#D9DEDC';x.lineWidth=1;x.fillRect(px,py,pillW,pillH);x.strokeRect(px+.5,py+.5,pillW-1,pillH-1);x.fillStyle=colour;x.fillRect(px,py,4,pillH);x.fillStyle='#3A4445';x.textAlign='center';x.textBaseline='middle';x.fillText(text,px+pillW/2+2,py+pillH/2);x.textAlign='left';x.textBaseline='alphabetic';x.restore()};
  const newPage=(title,subtitle='NVQ Evidence Portfolio')=>{const c=document.createElement('canvas');c.width=W;c.height=H;const x=c.getContext('2d'),[sectionName,sectionColour]=pdfSectionFor(title);x._sectionColour=sectionColour;x._sectionName=sectionName;x.fillStyle=WHITE;x.fillRect(0,0,W,H);x.fillStyle='#F3F5F4';x.fillRect(0,0,W,1);x.fillStyle=MUTED;x.font='600 14px Arial';x.fillText('APPRENTICE+ · NVQ EVIDENCE PORTFOLIO',M,84);drawPdfStamp(x,title);x.fillStyle=INK;x.font='700 36px Arial';fitText(x,clean(title),M,146,W-2*M-245,36);x.fillStyle=MUTED;x.font='500 17px Arial';x.fillText(clean(subtitle),M,184);x.fillStyle='#D9DEDC';x.fillRect(M,218,W-2*M,1);x.font='600 13px Arial';const tabW=Math.min(230,Math.max(125,x.measureText(sectionName.toUpperCase()).width+34)),tabX=W-M-tabW;x.fillStyle='#F6F7F6';x.fillRect(tabX,235,tabW,34);x.strokeStyle='#E2E6E4';x.lineWidth=1;x.strokeRect(tabX+.5,235.5,tabW-1,33);x.fillStyle=sectionColour;x.fillRect(tabX,235,3,34);x.fillStyle='#4E5859';x.textAlign='center';x.fillText(sectionName.toUpperCase(),tabX+tabW/2+2,257);x.textAlign='left';pages.push({canvas:c,ctx:x,colour:sectionColour,sectionName});return {c,x,y:296}};
  ...
  function sectionHeading(x,t,y){x.fillStyle=INK;x.font='700 24px Arial';x.fillText(clean(t),M,y);x.fillStyle=x._sectionColour||TEAL;x.fillRect(M,y+15,46,2);x.fillStyle='#E4E7E6';x.fillRect(M+58,y+15,W-2*M-58,1);return y+52}
  function meta(p,date,type,attempt,newEvidenceType=type){const {x}=p;let y=p.y;x.fillStyle='#FAFBFA';x.fillRect(M,y,W-2*M,118);x.strokeStyle='#E3E6E5';x.lineWidth=1;x.strokeRect(M+.5,y+.5,W-2*M-1,117);label(x,'Learner',M+22,y+28);value(x,profile.fullName,M+22,y+56,19,true);label(x,'Date submitted',M+570,y+28);value(x,date||'-',M+570,y+56,19,true);label(x,'Evidence type',M+22,y+86);value(x,type,M+180,y+86,17,true);label(x,'Attempt',M+570,y+86);value(x,String(attempt),M+650,y+86,17,true);if(isPortfolioBuildingEvidence(type)){x.fillStyle='#6D28D9';x.font='700 12px Arial';x.fillText('PORTFOLIO BUILDING · 0.2 GLH',M+22,y+108)}if(isNewEvidence(newEvidenceType,attempt))drawNewEvidenceStamp(x);p.y=y+148;return p}
  async function loadImage(src){if(!src)return null;return new Promise(r=>{const i=new Image();i.onload=()=>r(i);i.onerror=()=>r(null);i.src=src})}
